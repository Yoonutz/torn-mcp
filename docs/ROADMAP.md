...
| Deprecated Node-20 workflow actions                                        | ✅ Fixed | v0.9.9  | Bump checkout/setup-node to v6, upload-artifact to v7 (Node 24 runtime)                                          |
| `analyze_player` personalstats always null                                 | ✅ Fixed | v0.11.0 | Torn needs `cat`; now sent as `all`, and a failure is reported in `personalstats_error` instead of a silent null |
| `/property/{id}/property` missing from `torn_property`                     | ✅ Fixed | v0.11.0 | Catalog kept dropping parameterized paths whose last segment equals the tag                                      |
| API key accepted via `?key=` (could land in Cloudflare invocation logs)    | ✅ Fixed | v0.11.0 | Header only; query string not forwarded; invocation logs disabled in wrangler.toml                               |
| Item-name resolver broke numeric id lists and uid endpoints                | ✅ Fixed | v0.11.0 | `206,207` passes through; `itemdetails`/`itemstats` never name-resolve; mixed lists rejected                     |
| Sessions broke after a Durable Object restart (400 instead of 404)         | ✅ Fixed | v0.11.0 | Fresh DO answers 404 "Session not found" so clients re-initialize; verified with a forced `wrangler dev` restart |
| Merged id/no-id variants lost the id variant's params                      | ✅ Fixed | v0.11.0 | Catalog keeps `idQuery`/`idSummary`; validation targets the called variant and rejects unknown params            |
| Tool descriptions too long for clients (torn 19.7k chars)                  | ✅ Fixed | v0.11.0 | Long enums elided (+N more), return keys moved to discovery, ≤5000 chars enforced by a registration test         |
| Compile-only conformance marked baseline bugs "resolved"                   | ✅ Fixed | v0.11.0 | Resolution needs runtime evidence; otherwise "baseline not evaluated"                                            |
| Baseline refresh committed a catalog from an uncommitted spec              | ✅ Fixed | v0.11.0 | Refresh commits spec + all generated artifacts together; contract test checks the manifest specHash              |
